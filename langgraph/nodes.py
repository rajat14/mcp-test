...
async def planner_node(state: AgentState, tool_executor: ToolExecutor) -> AgentState:
    ...
    # Use existing prompt library
    prompt = tool_executor.prompt_library.retrieve_prompt_via_defaults(
        prompt_name="azure_openai/create_execution_plan_v2",   # can be changed
    ...
    logger.info("=== Full planner prompt sent to llm===")
    logger.info(prompt)
    try:
        # Call the external LLM service
        response = InferenceService().query_via_config(llm_connector_name="claude_vertex", prompt=prompt)
        ...
        logger.error(f"Planner Node Error: JSON/Parsig Failed {e}")
        # If planning fails, force a response action with an error message
        return {"next_action": {"action": "respond", "response": f"Planning failed due to a JSON parsing error: {str(e)}"}}
...
async def executor_node(state: AgentState, tool_executor: ToolExecutor) -> AgentState:
    ...
    if action and action.get("action") == "call_tool":
        ...
        logger.info(f"Executor Node: Executing tool '{tool_name}' and params '{params}")

        try:
            # Call the execution logic (API building, HTTP call)
            result = await tool_executor.execute_tool(tool_name, params)
            
            # Check for execution errors (eg: HTTP 404, connection failure)
