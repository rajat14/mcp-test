...
class ToolExecutor:
    ...
    async def execute_tool(self, tool_name: str, parameters: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"the query params are:{query_params}")
            request_body = parameters.get("request_body", {})
            logger.info(f"the request body is:{request_body}")

             # --- DEFENSIVE PARAMETER CORRECTION LOGIC ---
            if method == "get":
                
                # Scenario A: LLM put parameters directly into request_body (less likely for Get Lineage)
                if request_body and not request_body.keys() == {'lineage'}:
                    query_params.update(request_body)
                    self.logger.info("Queryparams are:{query_params}")

                # Scenario B: The confirmed issue - LLM nested parameters under a 'lineage' key
                elif request_body.get('lineage') and isinstance(request_body['lineage'], dict):
                
                    # Un-nest the parameters from the 'lineage' key and move them to query_params
                    lineage_params = request_body.pop('lineage')
                    query_params.update(lineage_params)
                
                    self.logger.info(f"DEFENSE: Flattened nested 'lineage' parameters for GET tool: {tool_name}")
            
            # Scenario C: Handle path parameters (original logic remains)
            # ... (original logic for path params using final_path goes here) ...

            # --- END DEFENSIVE PARAMETER CORRECTION LOGIC ---
        
            
            # Handle path parameters (todo: check and discuss this logic)
            final_path = path
            ...
            logger.info(f"Attempting {method.upper()} request to : {full_url}")
    ...
    async def synthesize_response(self, original_query: str, results: Dict[str, Any], execution_plan: Dict[str, Any]) -> str:
        ...
        try:
            ...
            response = InferenceService().query_via_config(
                llm_connector_name="claude_vertex",
                prompt=prompt
            )
