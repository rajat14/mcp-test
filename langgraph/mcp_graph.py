...
def build_mcp_graph(tool_executor_instance: ToolExecutor):
    ...
    # define asynchronous wraper functions

    async def run_planner(state:AgentState) -> AgentState:
        return await planner_node(state, tool_executor_instance)

    async def run_executor(state:AgentState) -> AgentState:
        return await executor_node(state, tool_executor_instance)

    async def run_synthesizer(state:AgentState) -> AgentState:
        return await synthesizer_node(state, tool_executor_instance)
