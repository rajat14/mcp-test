def filter_lineage_json(lineage_file_path, query_column=None, query_namespace=None, query_table=None, direction="downstream"):
    ...
    def add_job_and_columns(edge, next_fields, container, visited):
        """Helper to add jobs + next fields to results"""
        ...
        for nf in next_fields:
            field_info = extract_table_info(nf)
            if field_info and field_info not in container:
                container.append(field_info)
            find_downstream_lineage(nf, visited)  # continue recursion
    ...
    def find_downstream_lineage(start_field_id, visited=None):
        ...
        for edge in edges:
            ...
            # Case 1: OpenLineage style previous → next
            if start_field_id in previous_fields:
                add_job_and_columns(edge, next_fields, downstream_columns, visited)

            # Case 2: our JSON style (origin = field, mapping in next)
            elif edge.get("origin") == start_field_id:
                add_job_and_columns(edge, next_fields, downstream_columns, visited)
        ...
        for edge in edges:
            ...
            if target_field_id in next_fields:
                ...
                for pf in previous_fields:
                    field_info = extract_table_info(pf)
                    if field_info and field_info not in upstream_columns:
                        upstream_columns.append(field_info)
                    find_upstream_lineage(pf, visited)

            # Case 2: our JSON style (destination = field, mapping in previous)
            elif edge.get("destination") == target_field_id:
                for pf in previous_fields:
                    field_info = extract_table_info(pf)
                    if field_info and field_info not in upstream_columns:
                        upstream_columns.append(field_info)
...
user_query = "can you find the downstream lineage of column:c08c005, namespace:multi_entity_capital_enriched, table:capital_rrds_fact for 2 depths"

filtered_data = process_lineage_query("marquez_json_v1.json", user_query, "output_v1.json")
