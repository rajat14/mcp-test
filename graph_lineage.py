...
class OpenLineageAnalyzer:
    ...
    def load_merged_graph(self, path: Path) -> None:
        """Load a merged graph from gpickle file."""
        if not path.exists():
            raise FileNotFoundError(f"Graph file not found: {path}")
        self.merged_graph = nx.read_gpickle(str(path))
        self._build_indices()
    ...
    def find_nodes_by_pattern(self, dataset: Optional[str] = None, column: Optional[str] = None, 
        ...
        """Find nodes matching dataset/column patterns."""
        graph = self.merged_graph or nx.compose_all(self.per_event_graphs.values())
        matches = []
        
        for node, attrs in graph.nodes(data=True):
            parsed = attrs.get("parsed", {})
            node_dataset = parsed.get("dataset", "")
            node_column = parsed.get("column", "")
            
            if case_insensitive:
                node_dataset = node_dataset.lower()
                node_column = node_column.lower()
            ...
            dataset_match = not match_dataset or node_dataset == match_dataset
            column_match = not match_column or node_column == match_column
    ...
    def analyze_column_impact(self, dataset: str, column: str) -> Dict[str, Any]:
        ...
        for path in downstream["paths"]:
            ...
            if parsed.get("dataset"):
                affected_tables.add(parsed["dataset"])
            if parsed.get("column"):
                affected_columns.add(parsed["column"])
    ...
    def get_table_schema_evolution(self, dataset: str) -> Dict[str, Any]:
        ...
        for entry in entries:
            ...
            if graph_file in self.per_event_graphs:
                ...
                for node, attrs in graph.nodes(data=True):
                    parsed = attrs.get("parsed", {})
                    if parsed.get("dataset") == dataset and parsed.get("column"):
...
def search_columns(column_pattern: str, case_insensitive: bool = True) -> str:
    ...
    try:
        ...
        for node in matches:
            ...
            column = parsed.get("column", "unknown")
